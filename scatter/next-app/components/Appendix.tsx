...
const Appendix = ({ config, translator }: Props) => {
  ...
  return (
    <div id="appendix">
      ...
      <h2 className="text-3xl font-bold my-4 border-t-black">
        {t("Appendix")}
      </h2>
    ...
    <div key={step}>
      ...
      {conf.prompt && (
        <span>
          ...
          >
            {showPrompt ? t("hide prompt") : t("show prompt")}
          </span>
          ({conf.model})
        </span>
      )}
      <div>
        {showCode && (
          <div className="my-4 border rounded p-4 bg-gray-50">
            <pre className="text-xs">{conf.source_code}</pre>
          </div>
        )}
        {showPrompt && (
          <div className="my-4 border rounded p-4 bg-gray-50">
            <pre className="text-xs whitespace-pre-wrap">{conf.prompt}</pre>
          </div>
        )}
      </div>
    </div>
        ...
        {showChanges && (
          <ul className="text-left list-outside list-disc ml-6 mt-4">
            {changes.map((change: any, i: number) => (
              <li key={i}>
                {diffWords(change.from, change.to).map(
                  (part: any, j: number) => (
                    <span key={`${i}/${j}`}>
                      {part.added && (
                        <span className="bg-green-200">{part.value}</span>
                      )}
                      {part.removed && (
                        <span className="bg-red-200 line-through">
                          {part.value}
                        </span>
                      )}
                      {!part.added && !part.removed && (
                        <span>
                          {part.value.length < 163
                            ? part.value
                            : part.value.slice(0, 80) +
                              " ... " +
                              part.value.slice(part.value.length - 80)}
                        </span>
                      )}
                    </span>
                  )
